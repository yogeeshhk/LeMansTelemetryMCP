...
def register(mcp, runner):
    ...
    async def compare_corner(session_id: SessionId, corner_id: CornerId,
        ...
        """Use after get_corners on the first lap and summaries of 2-5 benchmark-candidate laps. Return the first lap as reference, matched corner metrics on later laps, and lap-minus-reference differences. Manual corners match by exact ID; automatic corners match nearest start within 100 m and can be unmatched. A uniquely associated sourced name includes source and calibration status without changing measured metrics; see get_track_guide. Distances are metres, speeds km/h, time seconds and steering percent. Brake point, turn-in, apex/minimum-speed position, throttle pickup/full throttle, entry/exit speed, approximate section time and ABS/TC activity are included where covered; nulls and quality flags limit conclusions. Check fuel/tyres and coverage before coaching; a positional or time difference alone does not establish cause."""
