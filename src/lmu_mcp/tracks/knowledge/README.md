...
## Monza review (2026-09-25)

`monza.json` matches only `Autodromo Nazionale Monza` / `Monza Curva Grande Circuit`. Its 11-feature order and character follow the [circuit operator description](https://www.monzanet.it/en/circuit/) and the operator's [2026 venue map](https://www.monzanet.it/wp-content/uploads/2026/01/Brochure_Hospitality_GP_2026F1.pdf). The pack uses the operator's current Biassono and Alboreto names while mentioning Curva Grande and Parabolica only as aliases. Unknown short, historic or renamed variants do not inherit the guide.

The available 2026-09-19 practice recording has no usable complete lap for distance calibration. Its complete interval contains a distance reversal, so the calibration report rejects it rather than sorting or repairing the samples; its other interval is incomplete. Consequently every feature, range and association in this pack is explicitly `approximate`. The broad metre ranges are map-guided estimates on the operator's stated 5793 m lap, with visible uncertainty. They are neither measured LMU turn boundaries nor braking markers. A future detected corner receives a name only through the existing unique single-corner association rule and retains `approximate` status until a reviewed pack revision replaces it.

The guide supplies an ordered reconnaissance lap and conditional technique cues without claiming personal corner metrics. LMU release notes support two narrowly scoped setup facts: the Oreca 07 LMP2+ ELMS package has a Monza medium-downforce option, and the Genesis GMR-001 LMDh received longer default gearing for this layout. These facts are not transferred to the recorded BMW Hypercar or to other classes. Exact wing, gearing, brake, pressure, kerb, fuel and tyre targets remain unsupported. On the current recording, `get_track_guide` remains available while numerical corner analysis preserves the actionable distance-path error. The read-only review left the source SHA-256, size, modification time and WAL state unchanged.
