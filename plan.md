...
- Phase 19, step 5 and phase/Milestone 4 acceptance complete: the complete suite collected 185 tests and passed with the same three expected skips (two unavailable Windows file-symlink checks and the opt-in real-file smoke); repeated full-suite runs had no failures. Wheel and sdist builds passed. Archive inspection found exact copies of `setup.py`, both history modules and the expanded La Sarthe pack, with no DuckDB/WAL telemetry. A separate read-only supplied La Sarthe smoke returned all 15 guide features and eight top-level notes, 13 measured reference corners, supported recent history with four selected/sample laps, 12 allowlisted current setup controls and zero experiments because the recorded car is outside the cited LMP3 applicability. Every selected lap used recorded `Lap Time`; no setup values were printed. The source SHA-256 remained `3e7f6300b2df5e7927e1bfa3ed8138fcb9636ab82647598f0f7b70f3695c923c`, size 38,006,784 bytes, mtime was unchanged and no WAL appeared. Representative La Sarthe, Spa and Monza guides retain their stated `calibrated`, `calibrated` and `approximate` pack statuses and explicit feature-level uncertainty; guide-first workflow, evidence separation, unsupported details and subsequent personal analysis passed the synthetic audit. Phase 19 and Milestone 4 are accepted. Public ngrok and actual ChatGPT behavior remain separate open checks and were not established by local protocol tests.

- Phase 19, step 4 complete: the guide-first content audit found that La Sarthe had a sourced/calibrated 15-feature map but no coaching notes, so the pack now provides one ordered explanatory driving note for every feature plus eight overview, setup-testing, race and practice notes. Guidance covers connected sequences, positioning, brake release, rotation, throttle/exit priorities, kerb uncertainty, tow/traffic, stint consistency and a staged practice plan without personal metrics, fixed markers, gears, speeds, setup values or strategy claims. Synthetic workflow acceptance verifies the full guide without personal fields, guide availability with an unusable distance path, exact-layout/no-pack behavior, explicit requested-track precedence in server instructions, withholding car-specific experiments for an unsupported car and a later successful personal corner-history request. The initial audit assertion incorrectly rejected the phrase that a range was *not* a prescribed braking point; replacing it with checks for the explicit no-marker/no-value language produced 10 passing La Sarthe/schema/workflow tests. Production pack validation reports 15 features, eight top-level notes and 15 feature notes; documentation and `git diff --check` passed. Full suite, distributions and read-only real-file acceptance remain.

- Phase 19, step 3 complete: registered the fourteenth read-only MCP tool, `get_corner_history`, with strict corner/scope schemas and a description covering recorded-time ranking, exact-layout/car bounds, uncertainty, setup-source gates and no writes. Server instructions and user documentation now route a general track request to the sourced full-lap guide before personal timing analysis, then use measured corners and `recent` history before bounded `general`; the workflow describes expected guide depth, missing-pack/path behavior and when setup direction must be withheld. Actual MCP clients over stdio and local Streamable HTTP initialized, discovered the annotated tool, called both scopes, preserved compact structured/text parity, stayed under 300 KB, omitted driver identity and received invalid-scope/corner/missing-session errors. The base client inventory calls all 14 tools; Phase 18 excursion protocol coverage remains included. Focused Phase 19/setup/service/MCP, Phase 18 MCP and base MCP suite: 26 passed. Four edited Markdown files passed balanced-fence and local-link validation after correcting the validator's repository-root path handling; `git diff --check` passed. Full suite, package/read-only real-file checks and the final guide-first content audit remain.

- Phase 19, step 2 complete: added transport-independent recorded-time selection, repeated entry/mid/exit evidence summaries and conservative source-gated one-setting experiment synthesis, plus direct-Python `get_corner_history` with bounded `recent`/`general` scopes. General history reuses the five-session/100-candidate/30-complete-lap bounds and exact track/layout/car match, selects up to three fastest distance-valid benchmark candidates and three disjoint slowest complete distance-valid laps using only positive recorded `Lap Time`, and discloses selection, timing source, flags, sample counts, unmatched/unsupported laps and truncation. Corner matching uses manual ID, unique sourced feature or a measured start within 100 m; path-deviation overlap remains unconfirmed context. Conditions and allowlisted current setup are returned without driver identity. Experiments require repeated clean samples, an available recorded setting and exact Ligier JS P325 or Ginetta G61 LT P325 EVO applicability from the official LMU LMP3 guide; TC indices remain feedback-only because integer direction is unverified. Weak evidence, wrong cars, unavailable setup and unsupported timing yield no setup direction. Synthetic core/service/setup/Phase 18 regression suite: 21 passed; the initial system-Python command could not import pytest, then the configured `.venv` run passed. MCP registration/client coverage, workflow documentation and final acceptance remain.

- Phase 19, step 1 complete: inspected only the structure/types of `CarSetup` across all 89 current recordings (37,381-38,333 characters each), then added a bounded 100,000-character reader and fixed 14-control allowlist. The parser requires explicit availability, matching producer key, finite raw value and declared min/max with value in range; it preserves bounded producer caption/display labels and omits unknown, unavailable or malformed entries. All current recordings parsed 14 controls. Per-wheel `WM_*` fields stay excluded because wheel mapping is independently unverified. Reviewed the official LMU LMP3 and traction-control guides: LMP3 directions are restricted to Ligier JS P325/Ginetta G61 LT P325 EVO and do not apply to the recorded ADESS or other classes; TC parameter meanings are general but recorded integer direction remains unverified. No private setup value entered source, tests, docs or logs. Focused setup/database suite: 26 passed. History selection, evidence synthesis, experiments and MCP coverage remain.

- Phase 18, step 4 and phase acceptance complete: full `.venv\Scripts\python.exe -m pytest -q` passed 163 tests with 3 expected skips. Wheel/sdist builds passed; both archives contain exact copies of the excursion analysis/tool modules and no DuckDB/WAL telemetry. A separate read-only Spa smoke analyzed seven complete distance-valid laps in `recent` with 99.93% time coverage, 35 sustained events and 13 hotspots; bounded `general` selected three exact layout/car recordings, analyzed 11 laps, retained one unsupported lap explicitly, and returned 56 events across 17 hotspots. Every result remained `unconfirmed`; SurfaceTypes and GPS stayed non-triggering, while two uniquely overlapping calibrated features supplied names. SHA-256/size/mtime/WAL signatures for all three accessed recordings were unchanged; the selected file SHA-256 was `204dc2532a115ff0b424b938f1a099a7e56cac4525c1758215469b1c897b395e`. The first archive-inspection attempt failed only because a Windows path retained an extra `src/` prefix; corrected inspection passed. Phase 18 accepted; Phase 19 corner history and setup experiments is next. No event is described as an official track-limit violation. Public ngrok and actual ChatGPT behavior remain separately open.

- Phase 18, step 3 complete: registered the thirteenth read-only MCP tool, `get_excursion_hotspots`, and updated server instructions to route repeated path-deviation questions through `recent` before bounded `general` history. Actual clients over stdio and local Streamable HTTP initialized, discovered the annotated tool, called both scopes with paged results and received invalid-scope, over-limit and missing-session errors; responses stayed below 300 KB and omitted driver identity. Focused MCP/service/detector/base-protocol suite: 22 passed. README and core/testing documentation now cover inputs, bounds, inclusion of flagged complete laps, gap handling, confidence and why SurfaceTypes/GPS cannot establish official violations. Full suite, package contents and read-only real-file acceptance remain.

- Phase 18, step 2 complete: added bounded `recent` and `general` aggregation to the direct Python service. Recent analyzes complete laps in the selected recording; general includes the selected recording plus at most four recent exact `TrackName`/`TrackLayout`/`CarName` matches, examines at most 100 discovery candidates and attempts at most 30 complete laps. Complete flagged laps contribute events and are disclosed, while partial laps are excluded and no pace benchmark is inferred. Results cluster same-side peaks within 100 m, report event/lap/session counts, coverage, unconfirmed confidence, optional unique sourced feature, pagination and explicit session/lap/page truncation. Synthetic regressions cover repeated and opposite-side events, short noise, missing samples, distance reversal, missing signals, partial/flagged laps, exact car/layout filtering, sourced Campus association and all cost/page bounds. Focused detector/service suite: 12 passed. MCP registration/protocol, user documentation and real-file acceptance remain.

- Phase 18, step 1 complete: reviewed the public simulator plugin header and all 89 current recordings before defining a predicate. `Path Lateral` and `Track Edge` are consistently 10 Hz metres; all 403,659 finite nonzero pairs used the same sign, matching the documented same-side edge relationship, while 18,238 samples exceeded the edge magnitude. Added canonical mappings and a direct-Python sustained path-deviation detector that validates monotonic distance, splits missing/source/clock gaps and side changes, excludes runs shorter than 0.3 s, bounds events and labels every result `unconfirmed`. `SurfaceTypes` remains non-triggering despite observed codes 0/2/4/5/6 because four-component wheel mapping/current-build semantics are unverified; GPS lacks a sourced boundary and is also non-triggering. Initial focused testing exposed one incorrect missing-sample expectation; correcting it produced 27 passing detector/schema tests. Documentation records that this is vehicle-centre evidence relative to a very approximate path, never an official track-limit violation. Recent/general aggregation, MCP coverage and final real-file validation remain.

- Phase 17, step 3 and phase acceptance complete: full `.venv\Scripts\python.exe -m pytest -q` passed 149 tests with 3 expected skips (two opt-in real-file tests and unavailable Windows file-symlink privilege). Wheel/sdist builds passed; both archives contain the exact validated Monza pack and no DuckDB/WAL telemetry. A separate read-only smoke on the supplied Monza recording returned the exact identity, all 11 approximate features and eight coaching notes; `get_corners` returned `distance_reversal`, while the calibration report produced zero lap reports and preserved lap 1 as unsupported for that code. Actual `compare_corner` stopped earlier with `ineligible_lap` because the recording has no benchmark-candidate pair; the eligible synthetic regression verifies that a reversed reference path returns `distance_reversal`. The source SHA-256, size, mtime and absent-WAL state were unchanged. Reviewed the guide for full-lap order, explicit uncertainty, evidence/applicability labels and car-specific setup limits. Phase 17 accepted; Phase 18 likely-excursion patterns is next. No current Monza corner metric, braking marker or universal setup target is claimed. Public ngrok and actual ChatGPT behavior remain separately open.

- Phase 17, step 2 complete: synthetic service tests confirm the exact Monza guide remains available independently of lap-distance validity, unknown variants return `no_pack`, uniquely associated Curva di Lesmo 1 names carry `approximate` through `get_corners` and `compare_corner`, and a reversed reference lap makes both numerical tools return `distance_reversal` without path repair. Actual MCP clients initialized over stdio and local Streamable HTTP, discovered the read-only guide tool, paged all 11 features, retained approximate labels, compared the synthetic named corner and received invalid-page/missing-session errors. The first run had three assertion failures because tests abbreviated the pack's operator name; corrected expectations then passed. Focused `python -m pytest tests/test_phase17_integration.py tests/test_phase17_pack.py tests/test_phase3_mcp.py -q`: 17 passed. README/core/testing docs now distinguish guide availability from unavailable current metrics. Final full-suite, package and supplied-recording checks remain.

- Phase 17, step 1 complete: added an exact-identity 11-feature Monza guide from the circuit operator description/map, with every range and future association visibly `approximate`, bounded uncertainty and conditional coaching for a full reconnaissance lap. LMU release-note setup facts remain restricted to the named Oreca 07 LMP2+ ELMS and Genesis GMR-001 LMDh applications and are not presented as BMW Hypercar settings. The supplied 2026-09-19 recording was inspected read-only: its only complete interval has a distance reversal and its other interval is incomplete, so no corner metric calibrated the pack and no source path was sorted or repaired. Original SHA-256, size, mtime and WAL state were unchanged. Exact-layout, unknown-variant, unique association, approximate propagation and explicit future replacement tests passed in a 17-test focused pack/coaching suite with one expected Windows symlink-privilege skip; pack validation, documentation links/fences and `git diff --check` passed. Service/protocol regression coverage and final full-suite/package/read-only checks remain.

- Phase 16, step 4 and phase acceptance complete: full `.venv\Scripts\python.exe -m pytest -q` passed 140 tests with 3 expected skips (two opt-in real-file tests and unavailable Windows file-symlink privilege). Wheel/sdist build passed; both archives contain the exact validated 13-feature Spa pack and no private telemetry. A separate read-only Spa smoke returned all eight top-level coaching notes, matched Campus in `compare_corner` on two benchmark laps, and confirmed original SHA-256/size/mtime/WAL state unchanged. Reviewed the 21 coaching notes for ordered lap coverage, reasons, source/evidence separation and applicability; five-lap calibration and actual local stdio/HTTP results are recorded above. Spa general content is implemented; the cross-track/server-wide workflow audit remains Phase 19. Phase 16 accepted; Phase 17 Monza is next. Eau Rouge/Raidillon and Blanchimont remain approximate; no wet optimal-driving or setup targets are claimed. Public ngrok and actual ChatGPT behavior remain separately open. Documentation structure/local links and `git diff --check` passed.

- Phase 16, step 3 complete: actual MCP stdio and local Streamable HTTP clients initialized, discovered tools, paged all 13 Spa features with coaching/source labels, compared the synthetic Campus corner and received invalid-page/missing-session errors. Added synthetic rain applicability, missing-speed nulls, guide availability despite distance reversal, source/coaching edit invalidation and unchanged metric checks. Focused `python -m pytest tests/test_phase16_integration.py tests/test_phase3_mcp.py -q`: 13 passed. README/core/testing docs now describe Spa and the bounded optional coaching response. Eight Markdown files passed fences/local links; `git diff --check` passed. Final complete suite, distribution contents and read-only Spa smoke remain; no public tunnel or actual ChatGPT behavior was exercised.

- Phase 16, step 2 complete: added the 13-feature sourced Spa pack and explanatory coaching for each feature plus overview/setup/race/practice notes. Operator map pages 3/4 and WEC/LMU sources were reviewed; five complete laps support five single-corner ranges, with three to five unique names per lap. Complexes remain contextual; Eau Rouge/Raidillon and Blanchimont are approximate. Fixed existing association behavior so split detections cannot repeat one name and merged ranges crossing two named corners stay unnamed. The initial merged-range regression failed, exposing the defect; after the fix the focused pack/calibration/guide/La Sarthe suite passed 16 tests. Five-lap read-only recheck left the original SHA-256/size/mtime/WAL state unchanged. Pack size and documentation fences/link syntax passed; `git diff --check` passed. Wet/gap/provenance and protocol regressions plus final full-suite/package checks remain.
