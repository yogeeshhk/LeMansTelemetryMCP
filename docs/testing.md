...
Phase 19 tests (`tests/test_phase19_setup.py`, `tests/test_phase19_history_core.py`, `tests/test_phase19_service.py`, `tests/test_phase19_mcp.py`) cover bounded allowlisted setup parsing, unavailable/malformed/per-wheel omission, recorded-time good/bad groups, mixed conditions, wrong-car source applicability, repeated and weak evidence, unconfirmed excursion overlap, unsupported timing, no-advice fallbacks and actual stdio/HTTP calls for both scopes and errors. Setup values are synthetic. These checks do not verify a universal setup direction or actual ChatGPT response quality.
