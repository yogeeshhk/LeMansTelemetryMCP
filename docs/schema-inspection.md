...
## Phase 19 recorded-setup review (2026-09-25)

All 89 current recordings contain a `CarSetup` JSON object between 37,381 and 38,333 characters. Structural inspection found producer fields `available`, `key`, `value`, `minValue`, `maxValue`, optional `caption`, and `stringValue` for each setup control. The review logged key paths and types only; no recorded setup value was written to tests, source, documentation or logs.

The application reads `CarSetup` through a dedicated 100,000-character metadata query and parses only 14 fixed global or axle-level controls: brake balance/pressure, front/rear aero, differential preload/power/coast, front/rear anti-roll bars, three traction-control maps, ABS map and steering lock. An entry must be explicitly available, have a matching producer key, finite numeric raw value and declared range, and satisfy `minimum <= value <= maximum`. Producer caption/display labels are retained when bounded and printable. Unknown, unavailable or malformed entries are omitted. Every current recording yielded all 14 allowlisted entries. Per-wheel `WM_*` fields are excluded because no independent component-to-wheel mapping has been verified.

The [official LMU LMP3 setup guide](https://lemansultimate.com/lmp3-quick-setup-guide/) names the Ligier JS P325 and Ginetta G61 LT P325 EVO and gives car-specific directions for preload, springs, ride height and front anti-roll bar. Those directions must not be applied to the recorded ADESS LMP3 or any GT3/Hypercar/LMP2. The [official LMU traction-control guide](https://guide.lemansultimate.com/hc/en-gb/articles/13182869047311-How-do-I-configure-my-traction-control-in-Le-Mans-Ultimate) explains the meanings of traction-control slip, power-cut and lateral-slip-angle parameters, but it does not establish how the recorded integer index maps to more or less intervention. Parsed controls therefore retain `direction_semantics: unverified`; no numerical or directional setup experiment follows from parsing alone.
