...
Start with the kind of coaching requested. For a general track guide, resolve the exact track/layout with `list_sessions` and `get_session_info`, then call `get_track_guide` and give the sourced full-lap guide without introducing personal timing claims. The guide should cover lap order, corner approach and exit priorities, linked sequences, overtaking/race considerations, setup themes, practice progression, evidence labels and calibration uncertainty where the reviewed pack supports them. A missing pack or unusable lap-distance path stays explicit; the guide does not depend on personal lap eligibility.

When the user asks to analyse their driving, continue with the progressive personal workflow below. Stop when the evidence answers the question. Server instructions and tool descriptions guide this sequence; clients still choose which tools to call.
...
For braking questions after the coarse comparison, call `get_braking_zones` on the coached and reference laps, then `compare_braking_zones` with the same thresholds. Review unmatched/partial zones and ABS coverage before claiming a difference. For a named corner question, inspect the already requested guide or call `get_track_guide` after confirming the exact layout, then check its source/calibration status. Call `get_corners` on the reference lap and use its `corner_id` in `compare_corner` with 2-5 candidates. If the user asks about repeated history or setup, call `get_corner_history` with `recent` first; use `general` only for its bounded exact-layout/car search. It ranks only positive recorded times, discloses flags and conditions, and withholds setup direction unless repeated clean evidence, current setting availability and an exact car-applicable source all agree. Automatic corner IDs are lap-local; the comparison matches by start distance within 100 m and reports unmatched laps. User manual names take priority; curated names require a unique sourced match. Unknown or approximate names do not prove a measured corner boundary. Read quality flags, ABS/TC coverage and reconstructed section-time caveats before proposing a driving experiment. This workflow has local MCP protocol coverage, not an actual ChatGPT or public ngrok integration test.
