...
For braking questions after the coarse comparison, call `get_braking_zones` on the coached and reference laps, then `compare_braking_zones` with the same thresholds. Review unmatched/partial zones and ABS coverage before claiming a difference. `compare_corner` remains a future Phase 13 tool; do not invent named corners. This workflow has local MCP protocol coverage, not an actual ChatGPT or public ngrok integration test.
